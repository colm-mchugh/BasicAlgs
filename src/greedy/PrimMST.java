 ...
 * contains no cycles, and is connected (includes all the vertices of G).
 * The Minimum Spanning Tree (MST) of G is the spanning tree that has the
 * least cost of all the spanning trees of G.
 * 
 * The algorithm depends on the following properties of a undirected graph G:
 * A cut A,B of G is a partition of its vertices into subsets A and B such that
 * for all v E A, v !E B and for all u E B, u !E A.
 * Empty Cut Lemma: G is not connected <=> there exists a cut A, B of G such that
 * 0 edges cross A and B.
 * Double Crossing Lemma: if a cycle C of G has an edge E1 that crosses a cut A,B
 * then it must have at least one other edge E2 that also crosses A,B.
 * Lonely Cut Lemma: if there is exactly one edge crossing a cut A,B then it 
 * cannot be part of a cycle.
 * Cut property: Given a cut A,B of G, if E is the cheapest edge that crosses 
 * A,B then E is in the MST of G. 
 * 
 */
public class PrimMST<T> {
    ...
    /**
     * slowMST(): return the Minimum Spanning Tree (MST) of g.
     *      MST = empty graph
     *      X = { a single vertex from g, randomly chosen } 
     *      while (X != g.V())
     *          (u,v) = cheapest edge of g s.t. u E X and v !E X
     *          MST += (u,v)
     *          X += v
     * 
     ...
     * are not in X need to be reentered into gMinusX because they now cross between
     * the cuts X and g - X.
     * 
     * @return 
     */
